...
class WordEmbeddingNew(nn.Module):
    """Word Embedding
    ...
    def __init__(self, ntoken, emb_dim, dropout, op=''):
        super(WordEmbeddingNew, self).__init__()
        self.op = op
    ...
    def init_embedding(self, np_file, tfidf=None, tfidf_weights=None):
        ...
        weight_init_clone = torch.zeros(weight_init.shape)
        ...
        if 'c' in self.op:
            weight_init_clone[:weight_init.size(0)] = weight_init.clone()
            self.emb_.weight.data[:self.ntoken] = weight_init_clone#weight_init.clone()
